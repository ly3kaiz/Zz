/* ================================================================
   ...
   مع إصلاح السقوط – لا يقترح أبداً اتجاهاً غير صالح
   ================================================================ */
...
/* ================================================================
   AI WORKER – النسخة المحسّنة (مع معالجة السقوط)
   ================================================================ */
const WORKER_SOURCE = `
"use strict";
...
function getSpawnProbs(maxTile) {
    if (maxTile >= 10) return [0.10, 0.15, 0.20, 0.25, 0.30];
...
function analyze(board) {
    ...
    // ---------- المرحلة الأولى: مع فلتر الأمان ----------
    const candidates = [];
    let hasValidSafe = false;
    for (let d = 0; d < 4; d++) {
        const [nb, changed] = applyMove(board, d);
        if (!changed) {
            candidates.push({ dir: DIRS[d], expect: -Infinity, mono: 0, combined: -Infinity, valid: false, safe: false });
            continue;
        }
        const safe = isSafeMove(board, d, 15);
        if (!safe) {
            candidates.push({ dir: DIRS[d], valid: false, combined: -Infinity, safe: false });
            continue;
        }
        hasValidSafe = true;
        ...
        candidates.push({ dir: DIRS[d], expect: expectScore, mono: avgSteps, combined, valid: true, safe: true });
    }

    // ---------- المرحلة الثانية: إذا لم توجد حركة آمنة نلغي الفلتر ----------
    if (!hasValidSafe) {
        candidates.length = 0; // إعادة تعيين
        for (let d = 0; d < 4; d++) {
            const [nb, changed] = applyMove(board, d);
            if (!changed) {
                candidates.push({ dir: DIRS[d], valid: false, safe: false });
                continue;
            }
            const expectScore = expectimax(nb, depth, false);
            let totalSteps = 0;
            ...
            const avgSteps = totalSteps / sims;
            const combined = expectScore * 0.6 + avgSteps * 400 * 0.4;
            candidates.push({ dir: DIRS[d], expect: expectScore, mono: avgSteps, combined, valid: true, safe: false });
        }
    }
    ...
    let confidence = 0;
    const usingSafety = hasValidSafe; // هل استخدمنا فلتر الأمان؟
    if (bestIdx >= 0) {
        ...
        if (!usingSafety) {
            // لم نستخدم الفلتر، فالحركة محفوفة بمخاطر أكبر
...
/* ================================================================
   GAME STATE & UI (مع تعديل لفحص صلاحية الحركة)
   ================================================================ */
const state = {
...
function handleWorkerMessage(e) {
    state.aiRunning = false;
    let result = e.data;
    // فحص إضافي: إذا كان هناك اتجاه مقترح ولكن لا يغير اللوحة، نعتبره غير صالح
    if (result && result.direction && result.direction !== "none") {
        const test = applyMove(state.board, result.direction);
        if (!test.changed) {
            result = { direction: null, confidence: 0 };
        }
    } else if (!result || !result.direction || result.direction === "none") {
        result = { direction: null, confidence: 0 };
    }
...
function renderAIPanel(result) {
    ...
    const arrow = (result && result.direction) ? arrows[result.direction] : '✕';
    const hasResult = !!(result && result.direction);
    setAIArrow(arrow, hasResult);
    const conf = (result && result.confidence) || 0;
    const fill = document.getElementById('confidence-fill');
    fill.style.width = conf + '%';
    document.getElementById('confidence-label').textContent = `Survival: ${conf}٪`;
    setAIStatus(hasResult ? `أفضل حركة: ${arrow}  (ثقة ${conf}٪)` : 'لا توجد حركة صالحة');
    highlightSuggestedBtn();
}
